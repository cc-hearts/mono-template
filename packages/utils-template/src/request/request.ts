...
import { objectConvertParams } from '../lib/convert'
import { isObject } from '../lib/bool'
enum requestType {
  GET = 'GET',
  POST = 'POST',
  PUT = 'PUT',
...
function request<T extends responseType>(
  url = '',
...
function requestMethod<T extends responseType>(
  url: string,
  type: requestType,
...
export function Get<T extends responseType = defaultType>(
  url: string,
  params?: params,
  requestInit: RequestInit = {}
): Promise<T> {
  const enCodeParams = objectConvertParams(params)
  const fullPath = `${url}?${enCodeParams}`
  return requestMethod(fullPath, requestType.GET, requestInit)
}

function postRequest<T extends responseType>(
  url: string,
  requestInit: RequestInit = {},
...
export function Post<T extends responseType = defaultType>(
  url: string,
  params?: params,
  requestInit: RequestInit = {}
): Promise<T> {
...
export function FormDataPost<T extends responseType = defaultType>(
  url: string,
  params?: params,
  requestInit: RequestInit = {}
): Promise<T> {
  ...
  if (params && isObject(params)) {
    enCodeParams = objectConvertParams(params)
  }
  return postRequest(
    url,
    Object.assign({}, requestInit, { body: enCodeParams }),
    'application/x-www-form-urlencoded'
  )
}
export function MultiPost<T extends responseType = defaultType>(
  url: string,
  params?: params,
  requestInit: RequestInit = {}
): Promise<T> {
  ...
  return postRequest(
    url,
    Object.assign({}, requestInit, { body: formData }),
    'multipart/form-data'
  )
}
